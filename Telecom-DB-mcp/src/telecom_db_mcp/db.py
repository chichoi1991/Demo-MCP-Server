...
def init_db() -> duckdb.DuckDBPyConnection:
    ...
    with _init_lock:
        ...
        # Use all available CPU cores for analytical query parallelism.
        # Override via DUCKDB_THREADS env var (e.g. when running multiple
        # uvicorn workers in the same container, lower this to avoid
        # oversubscribing the CPU).
        try:
            duckdb_threads = int(os.environ.get("DUCKDB_THREADS", "8"))
        except ValueError:
            duckdb_threads = 8
        conn.execute(f"PRAGMA threads={max(1, duckdb_threads)}")
